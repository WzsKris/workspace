...
def main():
    ...
    leg = 0  # standing position
    high_min, high_max = -1.2, -0.8    #-1.2 -- -0.8
    ...
    while not rospy.is_shutdown():
        ...
        # Update high and low positions for the current leg
        high_positions[current_leg].data += direction * 0.5
        #low_positions[current_leg].data += direction * 1.5 * -0.5
        low_positions[current_leg].data += direction * 1 * -0.5

        # Check if we need to change direction
        if high_positions[current_leg].data >= high_max or high_positions[current_leg].data <= high_min:
            direction *= -1
