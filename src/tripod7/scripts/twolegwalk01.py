...
def main():
    ...
    # Create publishers for all legs
    ...
    while not rospy.is_shutdown():
        for i in range(2):  # Publish for leg01 and leg02
            pubList[i * 3].publish(leg_positions[i])
            pubList[i * 3 + 1].publish(high_positions[i])
            pubList[i * 3 + 2].publish(low_positions[i])

        # Publish zero value for leg03
        pubList[6].publish(Float64(0.0))

        # Publish the dynamic values for high03 and low03
        pubList[7].publish(high_positions[2])
        pubList[8].publish(low_positions[2])
